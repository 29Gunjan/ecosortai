...
def download_model_from_cloud(model_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to download {model_name}: HTTP {response.status_code}")
            return False
...
def safe_load_model(model_path, model_name):
    try:
        ...
        if not os.path.exists(model_path):
            download_success = download_model_from_cloud(model_filename)
            if not download_success:
                print(f"Could not download {model_filename}, using fallback model")
